...
namespace QuanLyKhachSan.Forms
{
    public partial class FrmTraPhong : Form
    {
        ...
        private void cboDat_SelectedIndexChanged(object a, EventArgs e)
        {
            Tai();
        }
        ...
        private void btnThemDB_Click(object a, EventArgs e)
        {
            ...
            foreach (var x in db)
            {
                if (x.MaTienNghi == ma)
                {
                    MessageBox.Show("Tiện nghi đã có trong danh sách đền bù.");
                    return;
                }
            }
        ...
        {
            string phong = dgvPhong.CurrentRow != null ? Convert.ToString(dgvPhong.CurrentRow.Cells["SoPhong"].Value) : "";
            var k = s.LapPhieuDenBu(txtSoDB.Text.Trim(), V(cboDat), phong, DateTime.Now, V(cboNV), new List<DenBuItem>(db));
